        ...
        {props.schedules.map((s) => (
          <tr key={s.id} className="hover:bg-slate-50/50 transition-colors">
            <td className="px-6 py-4 font-medium text-slate-900 text-xs space-y-1">
              ...
              <div>
                Aviakompanija: {s.airlineCode} -{" "}
                {s.airlineName || getCompanyName(s.companyId)}
              </div>
            </td>
            ...
            <td className="px-6 py-4 text-xs space-y-1">
              <div className="flex flex-col">
                <span className="text-slate-600">
                  ...
                  {s.departureAirportName || getAirportName(s.airportId)}
                </span>
                ...
                <span className="text-slate-600">
                  ...
                  {s.arrivalAirportName || getAirportName(s.arrivalAirportId)}
                </span>
                {!!s.stopoverAirports?.length && (
                  <span className="text-xs text-slate-500 mt-1">
                    Tarpiniai:{" "}
                    {s.stopoverAirports
                      .map((stopover) => `${stopover.code} - ${stopover.name}`)
                      .join(", ")}
                  </span>
                )}
              </div>
            ...
            <td className="px-6 py-4 text-xs space-y-1">
              ...
              <div>
                Fakt. išv.:{" "}
                {new Date(s.actualDepartureTime || s.departureTime).toLocaleString(
                  "lt-LT"
                )}
              </div>
              <div>
              ...
              <div>
                Fakt. atv.:{" "}
                {new Date(s.actualArrivalTime || s.arrivalTime).toLocaleString(
                  "lt-LT"
                )}
              </div>
            </td>
            ...
            <td className="px-6 py-4 text-xs space-y-1">
              ...
              <div>Vietos: {s.availableSeatCount}/{s.seatCount}</div>
